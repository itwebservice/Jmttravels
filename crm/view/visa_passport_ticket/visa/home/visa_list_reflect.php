...
while($row_visa = mysqli_fetch_assoc($sq_visa)){
	...
	if($app_invoice_format == 4)
		$url1 = BASE_URL."model/app_settings/print_html/invoice_html/body/tax_invoice_html.php?invoice_no=$invoice_no&invoice_date=$invoice_date&customer_id=$customer_id&service_name=$service_name&basic_cost=$basic_cost&taxation_type=$taxation_type&service_tax_per=$service_tax_per&net_amount=$net_amount&service_charge=$service_charge&total_paid=$total_paid&balance_amount=$balance_amount&sac_code=$sac_code&branch_status=$branch_status&visa_id=$visa_id&pass_count=$pass_count&roundoff=$roundoff&$bsmValues&credit_card_charges=$credit_card_charges";
	else
		$url1 = BASE_URL."model/app_settings/print_html/invoice_html/body/visa_body_html.php?invoice_no=$invoice_no&invoice_date=$invoice_date&customer_id=$customer_id&service_name=$service_name&basic_cost=$basic_cost&taxation_type=$taxation_type&service_tax_per=$service_tax_per&net_amount=$net_amount&service_charge=$service_charge&total_paid=$total_paid&balance_amount=$balance_amount&sac_code=$sac_code&branch_status=$branch_status&visa_id=$visa_id&roundoff=$roundoff&credit_card_charges=$credit_card_charges&currency_code=$row_visa[currency_code]&canc_amount=$cancel_amount&bg=$bg";
	...
	$temp_arr = array( "data" => array(
		...
		<button data-toggle="tooltip" class="btn btn-info btn-sm" onclick="visa_display_modal('.$row_visa['visa_id'] .');btnDisableEnable(this.id)" id="visa_display_modal_btn_visa_view'.$row_visa['visa_id'].'" title="View Details"><i class="fa fa-eye" aria-hidden="true"></i></button>
