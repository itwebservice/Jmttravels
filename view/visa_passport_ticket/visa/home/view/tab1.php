	        	    ...
	        	    <span class="main_block">
	        	      ...
	        	      <div class='highlighted_cost'> <?php echo "<label>Net Total  <em>:</em></label> ".$sq_visa_info['visa_total_cost']; ?></div>
		        		...
		        		<span class="main_block">
		        		  <i class="fa fa-angle-double-right cost_arrow" aria-hidden="true"></i>
		        		  <?php
							$sq_credit = mysqli_fetch_assoc(mysqlQuery("SELECT sum(`credit_charges`) as sumc FROM `visa_payment_master` WHERE `visa_id`='$sq_visa_info[visa_id]' and `clearance_status`!='Cancelled' and clearance_status != 'Pending'"));
